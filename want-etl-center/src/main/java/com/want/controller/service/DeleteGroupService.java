/**
 * -------------------------------------------------------
 * @FileName：DeleteGroupService.java
 ...
 */
@Service
public class DeleteGroupService extends ResponseService<String, String> {
	@Autowired
	...
	protected void checkParameters(String parameters) throws Exception {
		if (StringUtil.isEmpty(parameters)) {
			throw ResponseExceptionFactory.createParameterEmpty("groupId不為空值.");
		}
	}
	...
	protected Void dataAccess(InputArgumentDTO argument, String parameters) throws Exception {
		jobService.deleteGroup(parameters);
		return null;
	}
